    ...
    context "when creating a credit transaction" do
      ...
      context "with concurrent transactions" do
        it "handles concurrent requests gracefully" do
          allow_any_instance_of(Transaction).to receive(:save).and_raise(ActiveRecord::LockWaitTimeout)
          ...
          expect(json_response["error"]).to include("Transaction failed due to concurrent modification. Please try again.")
        end
      end
